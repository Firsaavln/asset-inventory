"use client";

import React, { useState } from "react";
import { useRouter } from "next/navigation";
import Link from "next/link";
import { toast } from "sonner";
import { MANAGING_DIVISIONS } from "@/lib/constants";
import { createCategory } from "../actions";
import { 
  Tags, Building2, Save, Info, 
  Loader2, LayoutGrid 
} from "lucide-react";

export default function AddCategoryForm() {
  const [isSubmitting, setIsSubmitting] = useState(false);
  const router = useRouter();

  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    setIsSubmitting(true);
    
    // 1. Munculkan Toast Loading dulu di atas
    const toastId = toast.loading("Menyimpan kategori baru...");
    const formData = new FormData(e.currentTarget);

    try {
      await createCategory(formData);
      
      // 2. Ubah Toast Loading jadi Sukses
      toast.success("Kategori Tersimpan!", {
        id: toastId,
        description: `Kategori ${formData.get("category_name")} berhasil ditambahkan.`,
      });

      setTimeout(() => router.push("/categories"), 1000);
      
    } catch (error) {
      // 3. Ubah Toast Loading jadi Error kalau gagal
      toast.error("Gagal Menyimpan", {
        id: toastId,
        description: "Terjadi gangguan pada database.",
      });
      setIsSubmitting(false);
    }
  };

  return (
    <form onSubmit={handleSubmit} className="bg-white p-6 md:p-8 rounded-[2rem] border border-slate-200 shadow-sm relative overflow-hidden">
      
      {/* Dekorasi Background Indigo (Indikasi Tambah Baru) */}
      <div className="absolute top-0 right-0 w-32 h-32 bg-indigo-50 rounded-bl-full -z-10"></div>
      
      <div className="flex items-center gap-3 mb-8">
        <div className="p-2.5 bg-indigo-100 rounded-xl text-indigo-600">
          <LayoutGrid className="w-5 h-5" />
        </div>
        <h2 className="text-xl font-bold text-slate-800">Detail Kategori Baru</h2>
      </div>

      <div className="space-y-6">
        {/* NAMA KATEGORI */}
        <div className="space-y-2.5">
          <label className="text-sm font-bold text-slate-700 ml-1 flex items-center gap-2">
            <Tags className="w-4 h-4 text-indigo-500" /> Nama Kategori <span className="text-rose-500">*</span>
          </label>
          <input 
            type="text" 
            name="category_name" 
            required
            placeholder="Contoh: Hardware, Perangkat Jaringan, dll..." 
            className="w-full bg-slate-50 border border-slate-200 p-4 rounded-2xl focus:ring-4 focus:ring-indigo-500/20 focus:border-indigo-500 outline-none transition-all font-medium text-slate-700" 
          />
        </div>

        {/* DIVISI PENGELOLA */}
        <div className="space-y-2.5">
          <label className="text-sm font-bold text-slate-700 ml-1 flex items-center gap-2">
            <Building2 className="w-4 h-4 text-indigo-500" /> Divisi Pengelola <span className="text-rose-500">*</span>
          </label>
          <div className="relative">
            <select 
              name="owner_dept" 
              required 
              className="w-full bg-slate-50 border border-slate-200 p-4 rounded-2xl focus:ring-4 focus:ring-indigo-500/20 focus:border-indigo-500 outline-none font-medium text-slate-700 cursor-pointer appearance-none"
            >
              <option value="">-- Pilih Departemen --</option>
              {MANAGING_DIVISIONS.map((div) => (
                <option key={div.id} value={div.id}>{div.label}</option>
              ))}
            </select>
            <div className="absolute inset-y-0 right-0 pr-4 flex items-center pointer-events-none">
              <svg className="w-5 h-5 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 9l-7 7-7-7"></path></svg>
            </div>
          </div>
          <p className="text-xs font-medium text-slate-400 ml-1 flex items-center gap-1.5 mt-1">
            <Info className="w-3 h-3" /> Informasi ini menentukan alokasi aset pada dashboard utama.
          </p>
        </div>
      </div>

      {/* TOMBOL AKSI */}
      <div className="mt-10 pt-6 border-t border-slate-100 flex justify-end gap-4">
        <Link href="/categories" className="px-6 py-4 rounded-2xl font-bold text-slate-500 hover:bg-slate-100 transition-all">
          Batal
        </Link>
        <button 
          type="submit" 
          disabled={isSubmitting}
          className="flex items-center justify-center gap-2 bg-indigo-600 hover:bg-indigo-700 disabled:bg-indigo-400 text-white px-8 py-4 rounded-2xl font-bold shadow-lg shadow-indigo-200 transition-all active:scale-95 group"
        >
          {isSubmitting ? (
            <><Loader2 className="w-5 h-5 animate-spin" /> Menyimpan...</>
          ) : (
            <><Save className="w-5 h-5 group-hover:scale-110 transition-transform" /> Simpan Kategori</>
          )}
        </button>
      </div>
    </form>
  );
}